...
$ch2 = curl_init();
curl_setopt($ch2, CURLOPT_URL, 'https://sss.must.edu.tw/qry_selcos.asp');
...
$len=0;
for($j=1;$j<$i;$j=$j+6){
	if(strip_tags($match[0][$j])== "課程大綱"){$j=$j+2;}
	if($j>$i){break;}
	$data_array[$len] =[
	"科目類別"=>strip_tags($match[0][$j+2]),
	"開課班級"=>strip_tags($match[0][$j]),
	"課程名稱"=>strip_tags($match[0][$j+1]),
	"學分"=>strip_tags($match[0][$j+3]),
	"時數"=>strip_tags($match[0][$j+4]),
	"授課老師"=>strip_tags($match[0][$j+5])
	];
	$len++;
}
